...
impl Config {
    pub fn init() -> Self {
        Config {
            port: env::var("PORT")
                .map(|v| v.parse::<i32>().unwrap_or(8080))
            ...
            access_token_key_secret: env::var("ACCESS_TOKEN_KEY").expect("ACCESS_TOKEN_KEY must be set"),
            refresh_token_key_secret: env::var("REFRESH_TOKEN_KEY").expect("REFRESH_TOKEN_KEY must be set"),
        }
    }
}
